...
export const tryUnpackSign = async ({
  ...
  const signerKid: undefined | string | VerificationMethod = signerDidDocument.authentication?.find((k) =>
  ...
  const signerKey = signerDidDocument.verificationMethod?.find((v) => v.id === signerKidString)
